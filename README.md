...
## Running Experiments
For our experiments we will use the data located in https://github.com/sumaiyah/DNN-RE-data and you can find all our code in the `experiments/` directory of this repository. You can run recreate several of our cross-validation experiments using the provided executable `run_experiment.py` which offers the following options:
```bash
...
```
from the `experiments/` directory and that should generate the results reported in our paper (up to possible some small marginal differences due to non-determinism).
